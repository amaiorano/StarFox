#include "ImageFuncs.h"
#include "ImageData.h"
#include <math.h>
#include <stdexcept>
#include <cassert>

std::shared_ptr<UBYTE> ImageFuncs::LoadTGA(const std::string& strFileName, ImageInfo& rImageInfo)
{
	// These defines are used to tell us about the type of TARGA file it is
//	const UBYTE TGA_RGB	= 2;	// This tells us it's a normal RGB (really BGR) file
//	const UBYTE TGA_A	= 3;	// This tells us it's a ALPHA file
	const UBYTE TGA_RLE	= 10;	// This tells us that the targa is Run-Length Encoded (RLE)

	std::shared_ptr<UBYTE> pResultData;

	unsigned short iWidth = 0, iHeight = 0;	// The dimensions of the image
	UBYTE length = 0;		// The length in bytes to the pixels
	UBYTE imageType = 0;		// The image type (RLE, RGB, Alpha...)
	UBYTE bpp = 0;			// The bpp per pixel for the image (16, 24, 32)
	FILE *pFile = nullptr;		// The file pointer
	int iChannels = 0;		// The iChannels of the image (3 = RGA : 4 = RGBA)
	int iPitch = 0;			// The iPitch (iChannels * iWidth)
	int i = 0;				// A counter
		
	// This function loads in a TARGA (.TGA) file and returns its data to be
	// used as a texture or what have you.  This currently loads in a 16, 24
	// and 32-bit targa file, along with RLE compressed files.  Eventually you
	// will want to do more error checking to make it more robust.  This is
	// also a perfect start to go into a modular class for an engine.
	// Basically, how it works is, you read in the header information, then
	// move your file pointer to the pixel data.  Before reading in the pixel
	// data, we check to see the if it's an RLE compressed image.  This is because
	// we will handle it different.  If it isn't compressed, then we need another
	// check to see if we need to convert it from 16-bit to 24 bit.  24-bit and
	// 32-bit textures are very similar, so there's no need to do anything special.
	// We do, however, read in an extra bit for each color.

	// Open a file pointer to the targa file and check if it was found and opened 
	if ((pFile = fopen(strFileName.c_str(), "rb")) == nullptr) 
	{
		throw std::invalid_argument("Failed to open file" + strFileName);
	}
		
	// Read in the length in bytes from the header to the pixel data
	fread(&length, sizeof(length), 1, pFile);
	
	// Jump over one byte
	fseek(pFile,1,SEEK_CUR); 

	// Read in the imageType (RLE, RGB, etc...)
	fread(&imageType, sizeof(imageType), 1, pFile);
	
	// Skip past general information we don't care about
	fseek(pFile, 9, SEEK_CUR); 

	// Read the iWidth, iHeight and bpp per pixel (16, 24 or 32)
	fread(&iWidth,  sizeof(iWidth), 1, pFile);
	fread(&iHeight, sizeof(iHeight), 1, pFile);
	fread(&bpp,   sizeof(bpp), 1, pFile);
	
	// Now we move the file pointer to the pixel data
	fseek(pFile, length + 1, SEEK_CUR); 

	// Check if the image is RLE compressed or not
	if (imageType != TGA_RLE)
	{
		// Check if the image is a 24 or 32-bit image
		if (bpp == 24 || bpp == 32)
		{
			// Calculate the iChannels (3 or 4) - (use bpp >> 3 for more speed).
			// Next, we calculate the iPitch and allocate enough memory for the pixels.
			iChannels = bpp / 8;
			iPitch = iChannels * iWidth;
			pResultData.reset( new UBYTE[iPitch * iHeight], std::default_delete<UBYTE[]>() );
			auto* pData = pResultData.get();

			// Load in all the pixel data line by line
			for (int y = 0; y < iHeight; y++)
			{
				// Store a pointer to the current line of pixels
				unsigned char *pLine = &(pData[iPitch * y]);

				// Read in the current line of pixels
				fread(pLine, iPitch, 1, pFile);
			
				// Go through all of the pixels and swap the B and R values since TGA
				// files are stored as BGR instead of RGB (or use GL_BGR_EXT verses GL_RGB)
				for (i = 0; i < iPitch; i += iChannels)
				{
					UBYTE temp   = pLine[i];
					pLine[i]     = pLine[i + 2];
					pLine[i + 2] = temp;
				}
			}
		}
		// Check if the image is a 16 bit image (RGB stored in 1 unsigned short)
		else if (bpp == 16)
		{
			unsigned short pixels = 0;
			UBYTE r=0, g=0, b=0;

			// Since we convert 16-bit images to 24 bit, we hardcode the iChannels to 3.
			// We then calculate the iPitch and allocate memory for the pixels.
			iChannels = 3;
			iPitch = iChannels * iWidth;
			pResultData.reset( new UBYTE[iPitch * iHeight], std::default_delete<UBYTE[]>() );
			auto* pData = pResultData.get();

			// Load in all the pixel data pixel by pixel
			for (int i = 0; i < iWidth*iHeight; i++)
			{
				// Read in the current pixel
				fread(&pixels, sizeof(pixels), 1, pFile);
				
				// To convert a 16-bit pixel into an R, G, B, we need to
				// do some masking and such to isolate each color value.
				// 0x1f = 11111 in binary, so since 5 bpp are reserved in
				// each unsigned short for the R, G and B, we bit shift and mask
				// to find each value.  We then bit shift up by 3 to get the full color.
				b = static_cast<UBYTE>((pixels & 0x1f) << 3);
				g = static_cast<UBYTE>(((pixels >> 5) & 0x1f) << 3);
				r = static_cast<UBYTE>(((pixels >> 10) & 0x1f) << 3);
				
				// This essentially assigns the color to our array and swaps the
				// B and R values at the same time.
				pData[i * 3 + 0] = r;
				pData[i * 3 + 1] = g;
				pData[i * 3 + 2] = b;
			}
		}	
		// Else return a nullptr for a bad or unsupported pixel format
		else
		{
			throw std::invalid_argument("Bad or unsupported pixel format when loading file: " + strFileName);
		}
	}
	// Else, it must be Run-Length Encoded (RLE)
	else
	{
		// First, let me explain real quickly what RLE is.  
		// For further information, check out Paul Bourke's intro article at: 
		// http://astronomy.swin.edu.au/~pbourke/dataformats/rle/
		// 
		// Anyway, we know that RLE is a basic type compression.  It takes
		// colors that are next to each other and then shrinks that info down
		// into the color and a integer that tells how much of that color is used.
		// For instance:
		// aaaaabbcccccccc would turn into a5b2c8
		// Well, that's fine and dandy and all, but how is it down with RGB colors?
		// Simple, you read in an color count (rleID), and if that number is less than 128,
		// it does NOT have any optimization for those colors, so we just read the next
		// pixels normally.  Say, the color count was 28, we read in 28 colors like normal.
		// If the color count is over 128, that means that the next color is optimized and
		// we want to read in the same pixel color for a count of (colorCount - 127).
		// It's 127 because we add 1 to the color count, as you'll notice in the code.

		// Create some variables to hold the rleID, current colors read, iChannels, & iPitch.
		unsigned char rleID = 0;
		int colorsRead = 0;
		iChannels = bpp / 8;
		iPitch = iChannels * iWidth;

		// Next we want to allocate the memory for the pixels and create an array,
		// depending on the channel count, to read in for each pixel.
		pResultData.reset( new UBYTE[iPitch * iHeight], std::default_delete<UBYTE[]>() );
		auto* pData = pResultData.get();
		
		struct TgaPixel
		{
			UBYTE B, G, R, A;
		};
		TgaPixel tgaPixel;

		assert(iChannels >= 0 && iChannels <= 4);
		//std::unique_ptr<UBYTE[]> pColors2(new UBYTE [iChannels]);
		//UBYTE* pColors = pColors2.get();
		//UBYTE pColors[sizeof(float32) * 4]; // Max size

		// Load in all the pixel data
		while(i < iWidth*iHeight)
		{
			// Read in the current color count + 1
			fread(&rleID, sizeof(rleID), 1, pFile);
			
			// Check if we don't have an encoded string of colors
			if (rleID < 128)
			{
				// Increase the count by 1
				rleID++;

				// Go through and read all the unique colors found
				while(rleID)
				{
					// Read in the current color
					fread(&tgaPixel, sizeof(tgaPixel.R) * iChannels, 1, pFile);

					// Store the current pixel in our image array
					pData[colorsRead + 0] = tgaPixel.R;
					pData[colorsRead + 1] = tgaPixel.G;
					pData[colorsRead + 2] = tgaPixel.B;

					// If we have a 4 channel 32-bit image, assign one more for the alpha
					if (bpp == 32)
						pData[colorsRead + 3] = tgaPixel.A;

					// Increase the current pixels read, decrease the amount
					// of pixels left, and increase the starting index for the next pixel.
					i++;
					rleID--;
					colorsRead += iChannels;
				}
			}
			// Else, let's read in a string of the same character
			else
			{
				// Minus the 128 ID + 1 (127) to get the color count that needs to be read
				rleID -= 127;

				// Read in the current color, which is the same for a while
				fread(&tgaPixel, sizeof(tgaPixel.R) * iChannels, 1, pFile);

				// Go and read as many pixels as are the same
				while(rleID)
				{
					// Assign the current pixel to the current index in our pixel array
					pData[colorsRead + 0] = tgaPixel.R;
					pData[colorsRead + 1] = tgaPixel.G;
					pData[colorsRead + 2] = tgaPixel.B;

					// If we have a 4 channel 32-bit image, assign one more for the alpha
					if (bpp == 32)
						pData[colorsRead + 3] = tgaPixel.A;

					// Increase the current pixels read, decrease the amount
					// of pixels left, and increase the starting index for the next pixel.
					i++;
					rleID--;
					colorsRead += iChannels;
				}
			}
		}
	}

	// Close the file pointer that opened the file
	fclose(pFile);

	// Fill up output params
	rImageInfo.iChannels = iChannels;
	rImageInfo.imageSize = Size2d<int>(iWidth, iHeight);

	// Unfortunately, data in TGAs are stored in reverse order, so reverse the rows
	ReverseRows(pResultData.get(), rImageInfo);

	return pResultData;
}

std::shared_ptr<UBYTE> ImageFuncs::LoadRAW(const std::string& strFileName, int iWidth, int iHeight, int iChannels, ImageInfo& rImageInfo)
{
	FILE *pFile = nullptr;

	if ( (pFile = fopen(strFileName.c_str(), "rb")) == nullptr )
		throw std::invalid_argument("Failed to open file: " + strFileName);

	size_t iDataSize = iWidth*iHeight*iChannels;

	// Allocate buffer large enough for the data
	std::shared_ptr<UBYTE> pData( new UBYTE [iDataSize], std::default_delete<UBYTE[]>() );
    
	// The data is already stored in RGB order, so just read it in directly
	if ( fread(pData.get(), 1, iDataSize, pFile) != iDataSize )
		throw std::runtime_error("Failed to read from file: " + strFileName);

	// Close the file pointer that opened the file
	fclose(pFile);

	// Return ImageData object	
	rImageInfo.iChannels = iChannels;
	rImageInfo.imageSize = Size2d<int>(iWidth, iHeight);
	return pData;
}

void ImageFuncs::AddAlphaChannel(std::shared_ptr<UBYTE>& rpData, ImageInfo& rImageInfo, const Color4UB& color)
{
	const UBYTE BLACK = 0;
	const UBYTE WHITE = 255;
	
	// Make sure there is no alpha channel already
	if ( rImageInfo.GetChannels() == 4 )
		throw std::logic_error("Cannot add alpha channel because image already contains one");

	// Allocate a new buffer that includes the alpha channel
	int iNewSize = (rImageInfo.GetDataSize()/3) * 4;
	std::shared_ptr<UBYTE> pNewData( new UBYTE[iNewSize], std::default_delete<UBYTE[]>() );

	// Clear the entire buffer to white (including alpha channel)
	memset(pNewData.get(), WHITE, iNewSize);

	// Cycle through and fill up the new buffer, taking color key into account to
	// determine alpha channel value
	int iChunks = rImageInfo.GetDataSize() / 3;
	for (int i=0; i<iChunks; ++i)
	{
		UBYTE* pOldDataPtr = rpData.get() + (i * 3);
		UBYTE* pNewDataPtr = pNewData.get() + (i * 4);

		// Copy 3 bytes
		memcpy(pNewDataPtr, pOldDataPtr, 3);

		// If this value matches the input color key, set the alpha to black (transparent)
		if ( pNewDataPtr[0] == color.r && pNewDataPtr[1] == color.g && pNewDataPtr[2] == color.b )
			pNewDataPtr[3] = BLACK;
	}

	// Reassign the new data buffer and update data members
	rpData = pNewData;
	rImageInfo.iChannels = 4;
}

UBYTE* ImageFuncs::AddWhiteAlphaChannel(const UBYTE* pData, ImageInfo& rImageInfo)
{
	const UBYTE WHITE = 255;

	// Make sure there is no alpha channel already
	if ( rImageInfo.GetChannels() == 4 )
		throw std::logic_error("Cannot add alpha channel because image already contains one");

	UBYTE* pNewData = nullptr;
	try
	{
		// Allocate a new buffer that includes the alpha channel
		int iNewSize = (rImageInfo.GetDataSize()/3) * 4;

		pNewData = new UBYTE[iNewSize];

		// Clear the entire buffer to white (including alpha channel)
		memset(pNewData, WHITE, iNewSize);

		// Cycle through and fill up the new buffer, being careful not to touch the
		// alpha values
		int iChunks = rImageInfo.GetDataSize() / 3;
		for (int i=0; i<iChunks; ++i)
		{
			const UBYTE* pOldDataPtr = pData + (i * 3);
			UBYTE* pNewDataPtr = pNewData + (i * 4);

			// Copy 3 bytes
			memcpy(pNewDataPtr, pOldDataPtr, 3);
		}
	}
	catch (...)
	{
		delete [] pNewData;
		throw;
	}

	// Update output param and return new data
	rImageInfo.iChannels = 4;
	return pNewData;
}

void ImageFuncs::ReverseRows(UBYTE* pData, const ImageInfo& rImageInfo)
{
	int iHeight = rImageInfo.imageSize.h;
	int iPitch = rImageInfo.GetPitch();

	// Allocate temp buffer to hold reversed rows
	std::unique_ptr<UBYTE[]> pDataReversed( new UBYTE[rImageInfo.GetDataSize()] );

	// Loop and reverse the rows
	UBYTE* pData2 = pData + (iPitch*iHeight) - iPitch;
	for (int i=0; i<iHeight; ++i)
	{
		memcpy(pDataReversed.get() + i*iPitch, pData2, iPitch);
		pData2 -= iPitch;
	}

	// Now copy reversed rows back to input data buffer
	memcpy(pData, pDataReversed.get(), rImageInfo.GetDataSize());
}

UBYTE* ImageFuncs::GrowToPowerOf2(const UBYTE* pData, ImageInfo& rImageInfo)
{
	int iWidth = rImageInfo.GetWidth();
	int iHeight = rImageInfo.GetHeight();
	int iChannels = rImageInfo.GetChannels();

	// Calculate the exponents that would best fit the image
	// to the next power of 2
	int iWidthPower =  (int)ceil( log((float32)iWidth)  / log(2.0f) );
	int iHeightPower = (int)ceil( log((float32)iHeight) / log(2.0f) );

	// Calculate the best fit dimensions
	int iBuffWidth =  (int)pow(2.0f, iWidthPower);
	int iBuffHeight = (int)pow(2.0f, iHeightPower);

	// Do we need to grow the data?
	if ( iBuffWidth == iWidth && iBuffHeight == iHeight )
		return nullptr;

	//assert(iBuffWidth >= iWidth && iBuffHeight >= iBuffHeight && "Internal error: allocated buffer is incorrect size");

	UBYTE* pNewData = nullptr;	
	try
	{
		// _May_ point to new data if input data does not contain an alpha channel
		std::unique_ptr<UBYTE[]> pOldDataWithNewAlpha;

		// If image only contains 3 channels, then we must add an all
		// white alpha channel before growing it so that blending will
		// actually remove the grown part when rendering.
		bool bAddedAlpha = false;
		if ( iChannels == 3 )
		{
			// (NOTE: updating rImageInfo here!)
			pOldDataWithNewAlpha.reset( AddWhiteAlphaChannel(pData, rImageInfo) );
			
			iChannels = 4;
			bAddedAlpha = true;
		}

		// Allocate larger buffer
		int iNewDataSize = iBuffWidth * iBuffHeight * iChannels;
		pNewData = new UBYTE [iNewDataSize];

		// Clear entire buffer to black for alpha blending
		const UBYTE BLACK = 0;
		memset(pNewData, BLACK, iNewDataSize);

		// We need the new and old buffer pitches
		int newPitch = iBuffWidth * iChannels;
		int oldPitch = iWidth * iChannels; //rImageInfo.GetPitch();

		// Grab pointer to old data for copying. If we added an alpha channel,
		// use the data with the alpha channel, otherwise, just use the old
		// data (which already contained an alpha channel).
		const UBYTE* pOldData = (bAddedAlpha? pOldDataWithNewAlpha.get() : pData);

		// Copy data row by row, making sure not to touch the "grown" space
		for (int i=0; i<iHeight; ++i)
		{
			memcpy(pNewData + i*newPitch, pOldData + i*oldPitch, oldPitch);
		}
	}
	catch (...)
	{
		delete [] pNewData;
		throw;
	}

	// Update image info and return newly allocated data
	rImageInfo.imageSize.w = iBuffWidth;
	rImageInfo.imageSize.h = iBuffHeight;
	return pNewData;
}

bool ImageFuncs::IsPowerOf2(const Size2d<int>& rSize)
{
	float32 fRes;
	int iRes;

	// First width...
	fRes = log((float32)rSize.w) / log(2.0f);
	iRes = static_cast<int>(fRes);

	// If fractional part in result, then it's NOT a power of two
	if ( fRes != static_cast<float32>(iRes) )
		return false;

	// Now height...
	fRes = log((float32)rSize.h) / log(2.0f);
	iRes = static_cast<int>(fRes);

	if ( fRes != static_cast<float32>(iRes) )
		return false;

	// Both width and height are powers of two
	return true;
}
